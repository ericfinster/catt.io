...
module Make (R : ReductionScheme) = struct
  ...
  let rec unify stgy top l t u =
    ...
    | (tm,um) ->
       ...
       raise (Unify_error msg)) with Unify_error s -> raise (Unify_error s) (* raise (Unify_error (str "%s@,while unifying %a with %a" s pp_value (force_meta t) pp_value (force_meta u))) *)
